...
#                     [--budget "<spec>"] [--harness claude|codex|opencode|pi|cursor]
...
#
# --harness records the project's coding-harness preference (asked once, at
# onboarding) in data/projects/<name>/harness — applied to any task
# dispatched without an explicit harness, ahead of config/crew-harness
# (resolution: per-task request > project > crew-harness > detected).
#
...
cmd_add() {
  local name="$1" mode="$2" auto="$3" desc="$4" test_cmd="$5" lint_cmd="$6" budget_spec="$7" harness="$8"
  case "$name" in
...
main() {
  local name="" mode="gated" auto=0 desc="" test_cmd="" lint_cmd="" validate=0 budget_spec="" harness=""
  while [ $# -gt 0 ]; do
    case "$1" in
      -h | --help) usage; exit 0 ;;
      --validate) validate=1 ;;
      --auto) auto=1 ;;
      --mode | --desc | --test | --lint | --budget | --harness)
  ...
  cmd_add "$name" "$mode" "$auto" "$desc" "$test_cmd" "$lint_cmd" "$budget_spec" "$harness"
}
